...
Let's start by making some predictions involving the slope and variance of blood traits under different models of their relationship with range breadth and limits. 


```{r}
# establish relationships, simulate data for plasticity models
intercept1 <- 0
range_breadth <- seq(1000,3500, by=30)
slope1 <- 0.00001
sigma <- 0.003
eps <- rnorm(range_breadth, 0, sigma)
hyp1 <- intercept1 + range_breadth*slope1 + eps
model1 <- rep("plasticity increases with range breadth", length(range_breadth))
df1 <- cbind.data.frame(range_breadth, hyp1, model1)
colnames(df1) <- c("breadth", "plasticity", "model")
intercept2 <- 0.045
slope2 <- -0.00001
hyp2 <- intercept2 + range_breadth*slope2 + eps
model2 <- rep("plasticity constrains range breadth", length(range_breadth))
df2 <- cbind.data.frame(range_breadth, hyp2, model2)
colnames(df2) <- c("breadth", "plasticity", "model")
intercept3 <- 0.02
slope3 <- 0
hyp3 <- intercept3 + range_breadth*slope3 + eps
model3 <- rep("null hypothesis", length(range_breadth))
df3 <- cbind.data.frame(range_breadth, hyp3, model3)
colnames(df3) <- c("breadth", "plasticity", "model")
plasticity_df <- rbind.data.frame(df1,df2,df3)
plasticity_df$model <- as.factor(plasticity_df$model) 
plasticity_df$model <- factor(plasticity_df$model, levels=c("plasticity increases with range breadth",
                              "plasticity constrains range breadth",
                              "null hypothesis"))

# establish relationships, simulate data for variance models
q1 <- ggplot(plasticity_df, aes(x=breadth, y=plasticity)) +
  theme_bw() +
  theme(panel.grid = element_blank(),
  ...
  geom_point(pch=21, color="steelblue",stroke=1.1) +
  facet_wrap(~model, nrow=1)

# establish relationships, simulate data for plasticity models
elevation1 <- rnorm(83, mean=2250, sd=750)
y1 <- (elevation1-mean(elevation1))^2
variance1 <- jitter(1-(y1/max(y1)), amount=0.25)
df4 <- cbind.data.frame(elevation1, variance1)
df4$model <- rep("central marginal hypothesis", nrow(df4))
colnames(df4) <- c("elevation","variance","model")
#elevation2 <- runif(83, 0, 4500)
#y2 <- (elevation2-mean(elevation2))^2
#variance2 <- jitter(1-(y2/max(y2)), amount=0.25)
#df5 <- cbind.data.frame(elevation2, variance2)
#df5$model <- rep("symmetrical hypothesis", nrow(df5))
#colnames(df5) <- c("elevation","variance","model")
elevation3 <- runif(83, 0, 4500)
y3 <- ((elevation3-(mean(elevation3))+700)^3)
variance3 <- jitter(1-(y3/max(y3)), amount=0.25)
df6 <- cbind.data.frame(elevation3, variance3)
df6$model <- rep("asymmetrical hypothesis", nrow(df6))
colnames(df6) <- c("elevation","variance","model")
elevation4 <- runif(83, 0, 4500)
variance4 <- rnorm(83, mean=0.5, sd=0.1)
df7 <- cbind.data.frame(elevation4, variance4)
df7$model <- rep("null hypothesis", nrow(df7))
colnames(df7) <- c("elevation","variance","model")
var_df <- rbind.data.frame(df4,df6,df7)
var_df$model <- as.factor(var_df$model) 
var_df$model <- factor(var_df$model, levels=c("central marginal hypothesis",
                                              "asymmetrical hypothesis",
                                              "null hypothesis"))

# establish relationships, simulate data for variance models
q2 <- ggplot(var_df, aes(x=elevation, y=variance)) +
  theme_bw() +
  theme(panel.grid = element_blank(),
        strip.background = element_blank(),
        axis.text.y = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks = element_blank()) +
  geom_point(pch=21, color="red4",stroke=1.1) +
  facet_wrap(~model, nrow=1)

plot_grid(q1,q2,labels='AUTO',nrow=2)

```

Next, we're going to load our data, take a look at it, and make some filtering choices.  
...
slope_hb_draws <- slope_hb_draws %>% 
  group_by(.variable) %>% 
  median_hdi(.value, .width = c(.95, .89, .5)) %>%
  group_by(.variable) %>% 
...
p37 <- plot_grid(p37a, p37b, p37c, labels="AUTO", nrow=1)
